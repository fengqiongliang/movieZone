...
import com.pxe.iscsi.target.scsi.inquiry.PageCode;
...
public class InquiryCDB extends CommandDescriptorBlock {
    ...
    /**
     * An enable vital product data (EVPD) bit set to one specifies that the device server shall return the vital
     * product data specified by the PAGE CODE field.
     * <p>
     * If the EVPD bit is set to zero, the device server shall return the standard INQUIRY data. If the PAGE CODE field
     * is not set to zero when the EVPD bit is set to zero, the command shall be terminated with CHECK CONDITION status,
    ...
    public InquiryCDB (ByteBuffer buffer) {
        ...
        if (enableVitalProductData) {
             ...
             */
            if (vpdpn != VitalProductDataPageName.SUPPORTED_VPD_PAGES) addIllegalFieldPointer(2);// value should be 0x00
        }
    }
