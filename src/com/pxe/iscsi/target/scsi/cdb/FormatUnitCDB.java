...
public final class FormatUnitCDB extends CommandDescriptorBlock {
    ...
    /**
     * The format protection information (FMTPINFO) field in combination with the PROTECTION FIELD USAGE field
     */
     ...
     * defined in SBC-3. A LONGLIST bit set to one specifies that the parameter list, if any, contains a long parameter
     * list header.
     * <p>
     ...
     * (i.e. the PDU's data segment is empty).
     */
     ...
     * list shall be used in an addition to the existing list of defects.
     */
     ...
     * descriptors in the defect list.
     */
    ...
    public FormatUnitCDB (ByteBuffer buffer) {
        ...
        if (!formatData) {
             ...
             */
            if (formatProtectionInformation != 0) addIllegalFieldPointer(1, 7);
