...
public abstract class ModePage implements ISerializable {
    ...
    /**
     * This value plus {@link #pageLength} equals the length in bytes of a MODE PAGE using the SUB-PAGE FORMAT.
     */
    private static final int SUB_PAGE_FORMAT_PAGE_LENGTH_MODIFIER = 4;

    /**
     * This value plus {@link #pageLength} equals the length in bytes of a MODE PAGE using the PAGE_0 FORMAT.
     */
    private static final int PAGE_0_FORMAT_PAGE_LENGTH_MODIFIER = 2;

    /**
     * When using the MODE SENSE command, a parameters saveable (PS) bit set to one indicates that the mode page may be
     * saved by the logical unit in a nonvolatile, vendor specific location. A PS bit set to zero indicates that the
     * device server is not able to save the supporte parameters. When using the MODE SELECT command, the PS bit is
     * reserved.
     */
    private final boolean parametersSaveable;

    /**
     * A SubPage Format (SPF) bit set to <code>false</code>/zero indicates that the page_0 mode page format is being
     * used. A SPF bit set to <code>true</code>/one indicates that the sub_page mode page format is being used.
     */
    ...
    /**
     * This value plus {@link #pageLength} equals the length in bytes of the MODE PAGE.
     */
