package com.pxe.iscsi.target.scsi;
 ...
 * <p>
 * It may contain either {@link SenseData}, {@link IResponseData}, or (rarely) both. Some SCSI commands require that no
 * response data is sent (i.e. data segment length = 0) - in these cases the {@link #EMPTY_DATA_SEGMENT} shall be used.
 * <p>
 * Since the target must honor the buffer size the initiator has allocated for data returned in the data segment, the
 * {@link #serialize()} method will only write as many bytes to the passed {@link ByteBuffer} as specified during
 * initialization.
 * 
...
public final class ScsiResponseDataSegment {
    ...
    /**
     * The number of bytes the initiator has allocated for data sent in the data segment, i.e. the maximum number of
     * bytes of this object that may be transmitted.
     */
    private final int allocationLength;

    /**
     * The length of the {@link ScsiResponseDataSegment}, without considering any limitations due to
     * {@link #allocationLength}, or <code>-1</code>.
     * 
     * @see #uncroppedSize()
     */
     ...
     * 
     * @param senseData the sense data sent as the result of SCSI error
     * @param allocationLength number of bytes the initiator has allocated for data sent in the data segment
     */
     ...
     * 
     * @param senseData senseData the sense data sent as the result of SCSI error
     * @param responseData data requested by the initiator
     * @param allocationLength number of bytes the initiator has allocated for data sent in the data segment
     */
    private ScsiResponseDataSegment (final SenseData senseData, final IResponseData responseData, final int allocationLength) {
    ...
    /**
     * Returns a {@link ByteBuffer} containing a serialized representation of this object.
     * 
    ...
    public ByteBuffer serialize () {
        ...
        if (size == 0) return ByteBuffer.allocate(0);// empty data segment
    ...
    /**
     * The length of the {@link ScsiResponseDataSegment}, without considering any limitations due to
     * {@link #allocationLength}.
     * <p>
     * The returned value is calculated just once, and then stored in {@link #uncroppedSize}. This summation is
     * performed only if {@link #uncroppedSize} is negative.
     * 
     * @return length of the {@link ScsiResponseDataSegment}, without considering any limitations due to
     *         {@link #allocationLength}
     */
    ...
    /**
     * Indicates if any bytes have to be cropped during {@link #serialize()} calls. The method returns <code>true</code>
     * if and only if the serialized length of this objects exceeds {@link #allocationLength}.
     * 
     * @return <code>true</code> if and only if the serialized length of this objects exceeds {@link #allocationLength}
     */
    public boolean getResidualOverflow () {
        if (uncroppedSize() > allocationLength) return true;
        return false;
    }

    /**
     * Returns the number of bytes that had to be cropped due to the total length of all contained fields exceeding the
     * {@link #allocationLength}.
     * 
     * @return the total number of bytes that have to be cropped during serialization
     * @see #serialize()
     */
    public int getResidualCount () {
        if (getResidualOverflow()) return uncroppedSize() - allocationLength;
        return 0;
    }
}
