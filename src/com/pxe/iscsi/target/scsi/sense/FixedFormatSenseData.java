...
import com.pxe.iscsi.target.scsi.sense.senseDataDescriptor.senseKeySpecific.SenseKeySpecificData;
...
public class FixedFormatSenseData extends SenseData {
    ...
    /**
     * The minimum length in bytes of a serialized fixed format sense data object.
     */
    ...
    /**
     * A VALID bit set to <code>false</code> indicates that the {@link #information} field is not defined in SPC or any
     * other command standard. A VALID bit set to <code>true</code> indicates the INFORMATION field contains valid
     * information as defined in the SPC or a command standard.
     */
    ...
    /**
     * See the SBC-2 READ LONG, SBC-2 WRITE LONG, and SSC-2 READ commands and for examples of incorrect length indicator
     * (ILI) bit usage.
     */
    private final boolean incorrectLengthIndicator;

    /**
     * If {@link #valid} == <code>true</code> the INFORMATION field contains valid information as defined in the SPC or
     * SBC standard.
     */
    ...
    /**
     * Non-zero values in the FIELD REPLACEABLE UNIT CODE field are used to identify a component that has failed. A
     * value of zero in this field indicates that no specific component has been identified to have failed or that the
     * data is not available.
     * <p>
     * The format of this information is not specified by the SPC. Additional information about the field replaceable
     * unit may be available in the ASCII Information VPD page, if supported by the device server.
     */
    ...
    /**
     * The {@link #additionalSenseLength} field indicates the number of additional sense bytes that follow. The
     * additional sense length shall be less than or equal to 244 (i.e., limiting the total length of the sense data to
     * 252 bytes). If the sense data is being returned as parameter data by a {@link ScsiOperationCode#REQUEST_SENSE}
     * command, then the relationship between the {@link #additionalSenseLength} field and the CDB ALLOCATION LENGTH
     * field is defined in SPC-3.
     */
    ...
    public FixedFormatSenseData (final boolean valid, final ErrorType errorType, final boolean fileMark, final boolean endOfMedium, final boolean incorrectLengthIndicator, final SenseKey senseKey, final FourByteInformation information, final FourByteInformation commandSpecificInformation, final AdditionalSenseCodeAndQualifier additionalSenseCodeAndQualifier, final byte fieldReplaceableUnitCode, final SenseKeySpecificData senseKeySpecificData, final AdditionalSenseBytes additionalSenseBytes) {
        ...
        if (additionalSenseBytes != null) asl += additionalSenseBytes.size();
    ...
    public void serialize (ByteBuffer byteBuffer, int index) {
        ...
        // *** bytes 3 - 6 ***
        // information
        if (information != null) information.serialize(byteBuffer, index + INFORMATION_FIELD_INDEX);
