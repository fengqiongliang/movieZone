package com.pxe.iscsi.target.settings.entry;


import com.pxe.iscsi.target.TargetServer;
import com.pxe.iscsi.target.settings.KeySet;
import com.pxe.iscsi.target.settings.NegotiationStatus;
import com.pxe.iscsi.target.settings.NegotiationType;
import com.pxe.iscsi.target.settings.TextParameter;


/**
 * This {@link Entry} sub-class is used for managing and negotiation String parameters. The <i>value</i> part of the
 * corresponding <i>key-value</i> pairs can be made up of one or several comma-separated strings.
 * <p>
...
public final class StringEntry extends Entry {
     ...
     * @param supportedValues all values supported by the jSCSI Target
    ...
    protected boolean inProtocolValueRange (Object values) {
        ...
        for (String s : vals)
            if (!TextParameter.checkTextValueFormat(s)) return false;
        return true;
    }
    ...
    protected Object parseOffer (TargetServer target, String values) {
        ...
        if (negotiationType == NegotiationType.DECLARED && split.length > 1) return null;// protocol error
        return split;
    }
    ...
    @Override
    public Entry copy () {
        final StringEntry e = new StringEntry(keySet, negotiationType, use, negotiationStatus, supportedValues, (String) value);
