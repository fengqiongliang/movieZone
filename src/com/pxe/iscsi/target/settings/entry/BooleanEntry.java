...
import com.pxe.iscsi.target.settings.BooleanResultFunction;
 ...
 * <p>
 * During text parameter negotiation, boolean values are encoded as <code>Yes</code> and <code>No</code>, meaning
...
public final class BooleanEntry extends Entry {
     ...
     * @param negotiationValue together with the <i>resultFunction</i> parameter this value indicates the values
     *            supported by the jSCSI Target
     * @param resultFunction the result function to use during negotiations
    ...
    protected boolean inProtocolValueRange (final Object values) {
        if (values instanceof Boolean) return true;
    ...
    @Override
    public Entry copy () {
        final BooleanEntry e = new BooleanEntry(keySet, use, negotiationStatus, negotiationValue, resultFunction, (Boolean) value);
