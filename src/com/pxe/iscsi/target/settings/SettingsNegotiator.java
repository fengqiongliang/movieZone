 ...
 * former one is in charge of all {@link Entry} objects with connection-specific parameters, the latter is in charge of
 * all session-wide parameter {@link Entry} objects.
 * 
...
public abstract class SettingsNegotiator {
    ...
    /**
     * Returns a deep copy of the passed {@link List} with exact copies of all contained {@link Entry} objects.
     * 
     * @param entries the {@link List} to copy
     ...
     * 
     * @param commitChanges <code>true</code> if and only if the negotiated changes to the elements in {@link #entries}
    ...
    /**
     * Adds a properly initialized {@link Entry} object to {@link #entries} for every parameter managed by this
     * {@link SettingsNegotiator}.
     */
    protected abstract void initializeEntries ();

    /**
     * Checks constraints that cannot be checked at the {@link Entry} level, e.g. because they depend on the values of
     * multiple parameters.
     * 
     * @return <code>true</code> if everything is fine, <code>false</code> if that is not the case
     */
