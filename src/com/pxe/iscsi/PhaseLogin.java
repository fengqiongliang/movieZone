...
import com.pxe.iscsi.ENUM.Stage;
import com.pxe.iscsi.pdu.LoginRequest;
import com.pxe.iscsi.pdu.LoginResponse;
...
public class PhaseLogin {
   ...
   direct the negotiation (propose options).
	...
	private void LoginOperationalNegotiation(Socket socket,LoginRequest request)  throws Exception{
		...
		response.setExpCmdSN(request.getCmdSN());
		response.setMaxCmdSN(32);
		if(request.getParameter("HeaderDigest")!=null)response.setParameter("HeaderDigest", "None");
		if(request.getParameter("DataDigest")!=null)response.setParameter("DataDigest", "None");
		if(request.getParameter("ErrorRecoveryLevel")!=null)response.setParameter("ErrorRecoveryLevel", "0");
		if(request.getParameter("InitialR2T")!=null)response.setParameter("InitialR2T", "No");
		if(request.getParameter("ImmediateData")!=null)response.setParameter("ImmediateData", "Yes");
		if(request.getParameter("MaxRecvDataSegmentLength")!=null)response.setParameter("MaxRecvDataSegmentLength", "65536");
		if(request.getParameter("MaxBurstLength")!=null)response.setParameter("MaxBurstLength", "262144");
		if(request.getParameter("FirstBurstLength")!=null)response.setParameter("FirstBurstLength", "65536");
		if(request.getParameter("MaxConnections")!=null)response.setParameter("MaxConnections", "4");
		if(request.getParameter("DataPDUInOrder")!=null)response.setParameter("DataPDUInOrder", "Yes");
		if(request.getParameter("DataSequenceInOrder")!=null)response.setParameter("DataSequenceInOrder", "Yes");
		if(request.getParameter("DefaultTime2Wait")!=null)response.setParameter("DefaultTime2Wait", "2");
		if(request.getParameter("DefaultTime2Retain")!=null)response.setParameter("DefaultTime2Retain", "60");
		if(request.getParameter("MaxOutstandingR2T")!=null)response.setParameter("MaxOutstandingR2T", "16");
		//if(request.getParameter("MaxRecvDataSegmentLength")!=null)response.setParameter("MaxRecvDataSegmentLength", request.getParameter("MaxRecvDataSegmentLength"));
		//response.setParameter("DefaultTime2Wait", "0");
		//response.setParameter("MaxConnections", "4");
		//if(request.getParameter("DefaultTime2Retain")!=null)response.setParameter("DefaultTime2Retain", request.getParameter("DefaultTime2Retain"));
		System.out.println(response);
		os.write(response.toByte());
		//os.flush();
	}
