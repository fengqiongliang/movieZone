 ...
 */
	SoftIwareInterfaceID((byte) 0x84),	
 ...
 */
	Vendor((byte) 0xC0);
	    ...
	    private static Map<Byte , PageCode> mapping;
	    static {
	    	PageCode.mapping = new HashMap<Byte , PageCode>();
	        for (PageCode s : values()) {
	        	PageCode.mapping.put(s.value, s);
	        }
	    }
	    private PageCode (final byte newValue) {
	        value = newValue;
	    }
	    public final byte value () {
	        return value;
	    }
	    public static final PageCode valueOf (final byte value) {
	        return PageCode.mapping.get(value);
	    }
	}
		
	private byte op = (byte)0x12;
	private boolean EVPD;
	private byte pageCode;
	private byte[] allocateLength = new byte[2];
	private byte Control;
	public Inquiry(){}
	public Inquiry(byte[] CDB) throws Exception{
		if(CDB.length!=16)throw new Exception("illegic CDB Size , the proper length is 16");
		EVPD = (ByteUtil.getBit(CDB[1], 7) == 1);
		pageCode = CDB[2];
		System.arraycopy(CDB, 3, allocateLength, 0, allocateLength.length); 
		Control = CDB[5];
	}
	
	public CDBOpcode getOpcode() {
		return CDBOpcode.Inquiry;
	}
	public boolean getEVPD() {
		return this.EVPD;
	}
	...
	public PageCode getPageCode() {
		return PageCode.valueOf(this.pageCode);
	}
	public void setPageCode(PageCode pageCode) {
		this.pageCode = pageCode.value;
	}
	public short getAllocateLength() { 
		return ByteUtil.byteArrayToShort(this.allocateLength);
	}
	public void setAllocateLength(short allocateLength) {
		this.allocateLength = ByteUtil.shortToByteArray(allocateLength);
	}
	...
	public String toString(){
		...
		build.append(System.getProperty("line.separator")+" EVPD   : "+this.EVPD);
		build.append(System.getProperty("line.separator")+" PAGE CODE : "+PageCode.valueOf(this.pageCode));
		build.append(System.getProperty("line.separator")+" allocateLength : "+ByteUtil.byteArrayToShort(this.allocateLength));
	...
	public byte[] toByte(){
		...
		try {
			dos.writeByte(0x12); //Opcode
			dos.writeByte(this.EVPD?0x01:0x00);
			dos.writeByte(this.pageCode);
			dos.write(this.allocateLength);
			dos.writeByte(this.Control); 
			dos.write(new byte[]{0,0,0,0}); //Padding
			dos.write(new byte[]{0,0,0,0}); //Padding
			dos.write(new byte[]{0,0}); //Padding
	...
	public static void main(String[] args) throws Exception{
		Inquiry original = new Inquiry();
		original.setEVPD(true);
		original.setPageCode(PageCode.ExtendInquiryData);
		original.setAllocateLength((short)2);
		original.setControl((byte)3);
		System.out.println(original);
		byte[] data = original.toByte();
		Inquiry after = new Inquiry(data);
